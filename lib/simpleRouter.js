...
module.exports = function (options) {
	...
	if (options.controllers && options.controllers.length > 0) {
		...
		throw new Error('There is no controller in options');
	}
	...
	this.execReq = function (req, res) {
		...
		if (valuePair.length != 2) {
			...
			if (controllers[controllerName]) {
				if (controllers[controllerName][method][actionName.toUpperCase()]) {
					controllers[controllerName][method][actionName.toUpperCase()](req, res);
				}
			} else {
				throw new Error("can't not find the '" + controllerName + "' controller");
			}
		}
	}
}